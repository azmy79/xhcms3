...
namespace DTcms.Web.admin.users
{
    public partial class user_edit : Web.UI.ManagePage
    {
        ...
        private void ShowInfo(int _id)
        {
            ...
            //拓展信息
            var userExt = new DTcms.BLL.UserExt().GetModel(model.id);
            if (userExt != null)
            {
                txtCnName.Text = userExt.CnName;
                txtEnName.Text = userExt.EnName;
                ralCartType.SelectedValue = userExt.CartType.ToString();
                txtCartNum.Text = userExt.CartNum;
                txtCRAddress.Text = userExt.CRAddress;
            }
        }
        #endregion
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                if (!DoEdit(this.id) || !SaveUserExt(id))
                {
        ...
        /// <returns></returns>
        public bool SaveUserExt(int userID)
        {
            var userExtBll = new DTcms.BLL.UserExt();
            var userExt = userExtBll.GetModel(userID);
            ...
            if (userExt == null)
            {
                ...
                userExt.UserID = userID;
            }
            userExt.CnName = txtCnName.Text;
            userExt.EnName = txtEnName.Text;
            userExt.CartType = Convert.ToInt32(ralCartType.SelectedValue);
            userExt.CartNum = txtCartNum.Text;
            userExt.CRAddress = txtCRAddress.Text;
